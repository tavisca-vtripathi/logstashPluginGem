...
class LogStash::Outputs::CloudWatchLogs < LogStash::Outputs::Base
  ...
  config :log_group_name, :validate => :string, :default => "test"

  # The destination log stream
  config :log_stream_name, :validate => :string, :default => "test"                                 #:required => true
  ...
  #Value for cloudwatch metrics
  config :cw_metric, :validate => :number, :default => 1
  ...
  def register
    require "aws-sdk"
    @cwl = Aws::CloudWatch::Client.new(aws_options_hash) 
  ...
  public
  def receive(event)
    #@log_stream_name = event["ApplicationName"]
  ...
  def put_log_events(log_events)
    ...
    log_events.each do |event|
	  applicationEvent = JSON.parse(event[:message])
          new_log_group_name = applicationEvent["ApplicationName"]
	  if (applicationEvent["AdditionalInfo"]["corelation-id"]) then 
		new_log_stream_name = applicationEvent["AdditionalInfo"]["corelation-id"]
	  else
		new_log_stream_name = applicationEvent["TracingToken"]
	  end
    end
    puts new_log_stream_name
    puts new_log_group_name
    #new_log_stream_name = log_events["ApplicationName"]
    #begin
     # @logger.info("Sending #{log_events.size} events to #{new_log_group_name}/#{new_log_stream_name}")
      #@last_flush = Time.now.to_f
      if @dry_run
        log_events.each do |event|
          puts event[:message]
      ...
      if(!new_log_group_name)
	new_log_group_name = "AppNameNotFound"
      end
      response = @cwl.put_metric_data({
  namespace: "Platform-Exceptions", # required
  metric_data: [ # required
    {
      metric_name: new_log_group_name, # required
      value: @cw_metric,
      unit: "Count",
    },
  ],
